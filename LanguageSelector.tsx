...
const LanguageSelector: React.FC<Props> = ({ onSelect }) => {
  return (
    <div className="min-h-screen flex flex-col items-center justify-center bg-black p-6">
      <div className="bg-[#0a0a0a] border-l-4 border-red-700 p-10 max-w-md w-full text-center">
        ...
        <div className="grid gap-3">
          {['en', 'fr', 'ar'].map((lang) => (
            <button
              ...
              className="w-full py-4 px-6 bg-transparent hover:bg-red-700/10 text-slate-300 hover:text-red-500 transition-all border border-slate-900 hover:border-red-700 font-bold flex justify-between typewriter"
            >
              <span>{lang === 'en' ? 'English' : lang === 'fr' ? 'Français' : 'العربية'}</span>
              <span className="text-slate-700 uppercase">{lang}</span>
            </button>
          ))}
        </div>
      </div>
